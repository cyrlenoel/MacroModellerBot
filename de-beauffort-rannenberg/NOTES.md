...
| \(\sigma_e=0.58\) as SSJ Rouwenhorst **unconditional** SD of \(\log e\) | **Misses Table 1** (iMPC\(_1\approx0.24\), HtM\(\approx20\%\)) |
| \(\sigma_e=0.35\) used here | `[PROVISIONAL]` chosen to hit Table 1 iMPC\(_1\approx0.51\) and HtM\(\approx49.7\%\) given \(\beta_{HA}\) and \(r=1/\beta_{RA}-1\) |
| \(G/Y\approx0.214\) | `[PROVISIONAL` as a named parameter] uniquely implied by 12% import/GDP, \(\phi_d,\phi_t,\mu_f\), NX=0, \(Y=1\) |
| \(\phi_l\) | Table 2 = 0.6; we set \(\phi_l=N_{NT}/(N_{NT}+N_T)\approx0.617\) so both wage PCs hold at \(w=1\) (same targeting sentence as §4.1) |

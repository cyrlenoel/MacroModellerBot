...
%
% Figures are the PNGs already in ../output/ (Python QZ paths).
% A later PR adds ../output/slides/ with cleaned PDF/PNG versions
% (labels, H=12 on (a), a ``calibrated, not estimated'' tag).
% Change \irfdir only; do not search a local figures/ folder.
...
% Single swap point for slide-ready figures: ../output/slides/
\newcommand{\irfdir}{../output/}
\graphicspath{{\irfdir}}
...
\begin{frame}{(a) sterilised term premium}
  \begin{center}
    \includegraphics[width=\textwidth,height=0.60\textheight,keepaspectratio]%
      {\irfdir irf_a_tp_sterilised.png}
  \end{center}
  \vspace{-0.15em}
  {\scriptsize
...
\begin{frame}{(a) sterilised term premium: who cuts consumption}
  The consumption and mortgage panels are on the previous figure
  (\texttt{irf\_a\_tp\_sterilised.png}).
  Same $+100$~bp path, $H=12$.
  \begin{itemize}
    ...
    \item Output impact is $-0.391\%$ (trough $-0.55\%$ in quarter~4).
          House prices impact $-0.752\%$ (trough $-2.6\%$ in quarter~8).
          The interest bill dips on impact ($-0.034$~pp of GDP) and
          peaks at $+0.20$~pp in quarter~24.
  \end{itemize}
  {\scriptsize Checked against \texttt{output/success\_checks.txt}
  and \texttt{irf\_tp\_sterilised.csv}.
  The check prints output $-0.545\%$ and house prices $-2.597\%$;
  the headlines above are those troughs at two and one decimals.}
  \Calibrated
\end{frame}
...
\begin{frame}{(a) sterilised term premium against (b) path news}
  \begin{center}
    \includegraphics[width=\textwidth,height=0.26\textheight,keepaspectratio]%
      {\irfdir irf_a_vs_b_y_ph.png}
  \end{center}
  \vspace{-0.15em}
  {\scriptsize
  The file labels the dashed line ``short-rate news''.
  That line is \textbf{(b) path news}
  until the slide-ready figures rename it.}
  \begin{center}
  {\scriptsize
  ...
  Weight $1/(4D)$ on the current short rate means this 100~bp long-rate
  move needs a much larger Bank Rate path (peak $+560$~bp in quarter~4).
...
\begin{frame}{(a) sterilised term premium: interest-bill split}
  \begin{center}
    \includegraphics[width=0.88\textwidth,height=0.34\textheight,keepaspectratio]%
      {\irfdir irf_a_ib_decomposition.png}
  \end{center}
  \vspace{-0.35em}
  {\scriptsize
...
\begin{frame}{Takeaways}
  \begin{itemize}
    ...
    \item \textbf{(b) path news}, scaled to the same long-rate impact,
          ...
          On the comparison figure that path is labelled ``short-rate news''.
          \textbf{(c) sovereign} is not yet modelled.

...
%
% Figures in figures/ are plotted from the CSVs in ../output/
% (Python QZ paths of dynare/uk_long_rate.mod). No estimated IRFs.
...
\begin{frame}{What we are asking}
  ...
  \begin{itemize}
    ...
    \item \textbf{(c) sovereign.}
          A fiscal / sovereign gilt risk premium, often with sterling
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}
        ...
        \item \textbf{(c) sovereign:}
              gilt--FX risk. The same long-rate
...
\begin{frame}{Three shocks, and how (a) is sterilised}
  \textbf{(a) sterilised term premium.}
  $TP_t$ is an AR(1) with $\rho_{TP}=0.90$ (illustrative, not held fixed).
  The innovation is scaled so $R^L$ rises 100~bp on impact
  (about $0.252$ quarterly percent;
  \texttt{success\_checks.txt}).
  \vspace{0.25em}

  {\footnotesize
  Anticipated Bank Rate peg. The Taylor rule stays in the model.
  For the first $H$ quarters the monetary residual is set to
  $\varepsilon^{\mathrm{ster}}_t=-i^{TR}_t$, and $i^{\mathrm{path}}_t=0$, so}
  \begin{align*}
    i_t
      &= \rho_i i_{t-1}
      ...
      &\quad
       ...
       + \nu_t
       + \varepsilon^{\mathrm{ster}}_t
       = 0.
  \end{align*}
  {\footnotesize
  $\varphi_y=0.125$ is the annual convention, hence the factor $1/4$.
  Residuals are anticipated. The rule resumes after $H$.
  A permanent peg is not used: inflation would be undetermined.
  \textbf{Horizon $H=12$ quarters}
  (\texttt{H\_peg} in \texttt{src/calibration.py};
  \texttt{p.H\_peg = 12} in \texttt{dynare/params\_calibration.m};
  \texttt{H = 12} in \texttt{dynare/run\_uk\_long\_rate.m}).
  The figure window is 40 quarters, so the rule is back on for the last 28.

  \textbf{(b) path news} shifts $\nu_t$ inside the rule
  ($\rho_\nu=0.75$), not sterilised, scaled to the same $+100$~bp long rate.
  \textbf{(c) sovereign} is the UIP premium $\rho^{fx}_t$
  (\texttt{e\_rhofx} in the \texttt{.mod}). It is not fired.}
\end{frame}
...
\begin{frame}{UIP: a rise in $rer$ is a depreciation}
  ...
  \begin{itemize}
    ...
    \item Under \textbf{(a) sterilised term premium}, $i_{t}=0$ on the peg
          ...
          for (a).
    \item \textbf{(c) sovereign} enters as $\rho^{fx}_{t}$ and can flip
          the sign. The shock is in the \texttt{.mod} and is not fired,
          so there is no (c) IRF in \texttt{output/}.
...
\section{Results}
%----------------------------------------------------------------------
\begin{frame}{(a) sterilised term premium: activity, houses, the bill}
  \begin{center}
  ...
  \vspace{-0.35em}
  {\scriptsize
  $+100$~bp on $R^{L}$, peg $H=12$.
  From \texttt{output/success\_checks.txt}
  (CSV \texttt{irf\_tp\_sterilised.csv}):}
  \begin{itemize}
    \item Output: impact $-0.391\%$, trough $-0.545\%$ in quarter~4.
    \item House prices: impact $-0.752\%$, trough $-2.597\%$ in quarter~8.
    \item Interest bill: impact $-0.0340$~pp of GDP, then a peak of
          $+0.2017$~pp in quarter~24.
          A full immediate refix of the conventional stock would be
          $0.75$~pp. The early dip is the index-linked uplift.
  \end{itemize}
  \Calibrated
\end{frame}
...
\begin{frame}{(a) sterilised term premium: who cuts consumption}
  \begin{center}
    \includegraphics[width=\textwidth,height=0.44\textheight,keepaspectratio]%
      {irf_a_consumption.png}
  \end{center}
  \vspace{-0.3em}
  {\scriptsize Same path. Savers in blue, borrowers in red on the left panel.}
  \begin{itemize}
    \item Savers do not raise consumption:
          $C^{s}\in[-0.768\%,-0.081\%]$ over the 40 quarters.
    \item Borrowers fall at least as much:
          $C^{b}\in[-1.038\%,-0.227\%]$,
          trough $-1.038\%$ in quarter~9.
    \item The lag is $\lambda_{q}$. New-loan $R^{m}$ jumps 70~bp on impact;
          the stock rate only 6.3~bp, and peaks at quarter~10,
          one quarter after the borrower trough.
  \end{itemize}
  \Calibrated
\end{frame}
...
\begin{frame}{(a) sterilised term premium against (b) path news}
  ...
  \begin{tabular}{@{}lcc@{}}
    ...
    Output trough & $-0.545\%$ (q4) & $-1.178\%$ (q5) \\
    House-price trough & $-2.597\%$ (q8) & $-6.551\%$ (q7) \\
    \bottomrule
  \end{tabular}
  }
  \end{center}
  \vspace{0.05em}
  {\scriptsize
  (b) from \texttt{irf\_short\_rate\_news.csv}.
  The expectations-hypothesis weight on the current short rate is only
  $1/(4D)$, so the same 100~bp long-rate move needs a much larger
  Bank Rate path (peak $+560$~bp in quarter~4).
  Under (b), $R^{L}$ later dips to $-11.8$~bp (quarter~16);
  under (a) it stays positive (minimum $+2.35$~bp).
  \textbf{(c) sovereign} is not on this figure.}
  \Calibrated
\end{frame}
...
\begin{frame}{(a) sterilised term premium: interest-bill split}
  \begin{center}
    \includegraphics[width=0.90\textwidth,height=0.40\textheight,keepaspectratio]%
      {irf_a_ib_split.png}
  \end{center}
  \vspace{-0.35em}
  {\scriptsize
  ...
  \begin{itemize}
    ...
    \item Sum: impact $-0.0340$~pp, peak $+0.2017$~pp in quarter~24.
...
\begin{frame}{Takeaways}
  \begin{itemize}
    ...
    \item In the \textbf{calibrated} twin, a $+100$~bp
          ...
          and house prices and, after an index-linked dip, raises the
          interest bill. Borrowers cut consumption by at least as much
          as savers, with a lag of $\lambda_{q}=0.09$.
          Savers do not raise consumption.
    \item \textbf{(b) path news}, scaled to the same long-rate impact,
          is a much larger short-rate path and a deeper contraction.
          \textbf{(c) sovereign} is coded and not fired.
...
%----------------------------------------------------------------------
\begin{frame}{Appendix: (a) sterilised term premium, prices and sterling}
  \begin{center}
  ...
  \vspace{-0.25em}
  {\scriptsize
  From \texttt{irf\_tp\_sterilised.csv}.
  Real exchange rate on impact: $-1.875\%$
  (a fall in $rer$ is an appreciation), the sign the review assigns to
  \textbf{(a) sterilised term premium}.
  Inflation: $-26.4$~bp annualised on impact, trough $-34.9$~bp in
  quarter~3. Business investment falls $0.505\%$ on impact, which is
  also its trough on this path.
  \textbf{(c) sovereign} is not shown.}
  \Calibrated
\end{frame}
...
\begin{frame}{Appendix: peg length is not doing the work}
  ...
  $H=40$ is robustness. $\kappa=0.004$ and $\psi_{\mathrm{nfa}}$ are what
  keep $R^{L}$ from crossing zero on a short peg; both are provisional.}
  \Calibrated
\end{frame}
...
\begin{frame}{Appendix: what the MVP leaves out}
  ...
  \end{itemize}
  \textbf{Coded and not fired}
  \begin{itemize}
    \item \textbf{(c) sovereign} ($\rho^{fx}$).
          Cost-push, government spending, foreign demand and the
          mortgage wedge $\zeta$ are AR shocks in the \texttt{.mod},
          unused in the (a)/(b) paths.
  \end{itemize}
